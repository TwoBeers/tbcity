...
class tbcity_Mobile {
	...
	function __construct () {
		...
		$tbcity_is_mobile = $this->is_mobile = apply_filters( 'tbcity_filter_is_mobile', $this->device_detect() ); // check if is mobile browser
	...
	function device_detect() {
		...
		if ( ! $this->get_option( 'mobile_css' ) ) return false;
	...
	function init () {
		...
		add_action( 'tbcity_mobile_hook_comments_before'	, array( $this, 'comments_navigation' ) );
		add_action( 'tbcity_mobile_hook_comments_after'	, array( $this, 'comments_navigation' ) );
		add_action( 'tbcity_mobile_hook_entry_before'		, array( $this, 'posts_navigation' ) );
		add_action( 'tbcity_mobile_hook_entry_after'		, array( $this, 'posts_navigation' ) );
		add_action( 'tbcity_mobile_hook_entry_after'		, array( $this, 'page_hierarchy' ) );
		add_action( 'tbcity_mobile_hook_content_before'	, array( $this, 'search_reminder' ) );
		add_action( 'tbcity_mobile_hook_content_after'		, array( $this, 'indexes_navigation' ) );
		...
		add_filter( 'tbcity_mobile_filter_seztitle'		, array( $this, 'get_seztitle' ) );
		...
		add_filter( 'tbcity_filter_taxomony_separator'		, array( $this, 'taxomony_separator' ), 90 );
		add_filter( 'tbcity_hook_get_the_thumb'			, array( $this, 'get_the_thumb' ), 10, 4 );
	...
	function setup() {
		...
		register_nav_menus( array( 'mobile' => __( 'Navigation Menu for mobiles<br /><small>only supports the first level of hierarchy</small>', 'tbcity' ) ) );
	...
	function widget_area_init() {
		...
		register_sidebar( array(
			'name'				=> __( 'Mobile Widget Area', 'tbcity' ),
			'id'				=> 'tbm-widget-area',
			'description'		=> '',
	...
	function stylesheet(){
		...
		wp_enqueue_style( 'tbm-mobile-style', get_template_directory_uri() . '/mobile/style-mobile.css', false, tbcity_get_info( 'version' ), 'screen' );
	...
	function indexes_navigation(){
		...
		echo apply_filters( 'tbcity_mobile_filter_seztitle', sprintf( __( 'page %1$s of %2$s', 'tbcity' ) , $paged, $wp_query->max_num_pages ) );
			...
			?>
				<div class="tbm-index-navi">
					<?php if ( function_exists( 'wp_pagenavi' ) ) { ?>
						<?php wp_pagenavi(); ?>
					<?php } else { ?>
								<?php previous_posts_link( __( 'Previous page', 'tbcity' ) ); ?>
								<?php next_posts_link( __( 'Next page', 'tbcity' ) ); ?>
					<?php } ?>
				</div>
			<?php
	...
	function page_hierarchy(){
		...
		if ( !empty($sub_pages) ) {
			...
			?>
				<?php echo apply_filters( 'tbcity_mobile_filter_seztitle', __( 'Child pages', 'tbcity' ) ); ?>
				<ul class="tbm-group">
					<?php 
		...
		if ( $parent_page ) {
			?>
				<?php echo apply_filters( 'tbcity_mobile_filter_seztitle', __( 'Parent page', 'tbcity' ) ); ?>
				<ul class="tbm-group">
	...
	function search_reminder() {
		...
		if ( is_archive() ) {
			...
			if ( is_category() || is_tag() || is_tax() ) {
				if ( is_category() )	$type = __( 'Category', 'tbcity' );
				elseif ( is_tag() )		$type = __( 'Tag', 'tbcity' );
				elseif ( is_tax() )		$type = __( 'Taxonomy', 'tbcity' );
				$title = $term->name;
			} elseif ( is_date() ) {
				$type = __( 'Date', 'tbcity' );
				if ( is_day() ) {
			...
			$text = sprintf( __( 'Search results for &#8220;%s&#8221;', 'tbcity' ), '<span class="search-term">' . esc_html( get_search_query() ) . '</span>' );

		} elseif ( is_404() ) {

			$text = __( 'Error 404', 'tbcity' );

		}

		echo apply_filters( 'tbcity_mobile_filter_seztitle', $text );
	...
	function comment_form_defaults( $defaults ) {
		...
		$defaults['title_reply']		= apply_filters( 'tbcity_mobile_filter_seztitle', __( 'Leave a comment', 'tbcity' ) );
		$defaults['title_reply_to']		= apply_filters( 'tbcity_mobile_filter_seztitle', __( 'Leave a Reply to %s', 'tbcity' ) );
	...
	function body_classes( $classes ) {
		...
		$classes[] = $this->get_option( 'mobile_css_color' );
